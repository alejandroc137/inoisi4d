...
static void hdf5_join_path(char *dst, size_t dst_size, const char *base, const char *name, const char *fn_name)
{
  int n = snprintf(dst, dst_size, "%s%s", base, name);
  if (n < 0 || (size_t)n >= dst_size) {
    fprintf(stderr, "HDF5 path is too long in %s while processing %s\n", fn_name, name);
    exit(EXIT_FAILURE);
  }
}
...
hdf5_blob hdf5_get_blob(const char *name)
{
  ...
  char path[STRLEN];
  hdf5_join_path(path, sizeof(path), hdf5_cur_dir, name, "hdf5_join_path");
...
{
  int n = snprintf(hdf5_cur_dir, sizeof(hdf5_cur_dir), "%s", path);
  if (n < 0 || (size_t)n >= sizeof(hdf5_cur_dir)) {
    fprintf(stderr, "HDF5 current directory path is too long: %s\n", path);
    exit(EXIT_FAILURE);
  }
}
...
{
  char path[STRLEN];
  hdf5_join_path(path, sizeof(path), hdf5_cur_dir, data_name, "hdf5_join_path");
